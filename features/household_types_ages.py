...
class HouseholdTypesAges(Feature):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if decennial_census_year == 2020:
            ...
            box_url = "https://bloombergdotorg.box.com/s/wzys9vuhjqol5mm47bv4ee92t1a0c2yi"
    ...
    def load_data(self):
        ...
        data = (
            ...
            .rename(columns=cols)
            .assign(geo_id=
            lambda x: x.geo_id.str.split("US").apply(lambda s: s[1]))
        )
        del cols['GEO_ID']
    ...
    def construct_feature(self, target_geo_grain: str) -> pd.Series:
        household_types_ages = (
                ...
                .groupby("geo")[self.features].sum()
                .reindex(self.index)
            )
        
        if self.verbose:
            print(f"{household_types_ages.isna().sum()} of {household_types_ages.shape[0]} {target_geo_grain}s are unaccounted for")
        return household_types_ages
