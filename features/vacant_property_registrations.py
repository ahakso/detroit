...
class VacantPropertyRegistrations(Feature):
    ...
    TYPES_vacant_properties = [float, float, int, str, str]
    ...
    def load_data(
        ...
        df = pd.read_csv(
            ...
            dtype=dict(zip(self.COLS_vacant_properties, self.TYPES_vacant_properties)),
        )

        registrations = gpd.GeoDataFrame(
            df, geometry=gpd.points_from_xy(df.lat, df.lon), crs="epsg:4326"
        ).rename(columns={"ObjectId": "oid"})
        registrations = registrations.assign(
